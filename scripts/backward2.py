...
def data(Param, fcf_backward, sol_vol, iteration, sol_lvl, stochastic):
    ...
    # set of biomass units
    model.Biomass = pyomo.Set(initialize= biomassPlants)
    ...
    # coeficcient of lineal segments in the future cost function
    model.coefcBio = pyomo.Param(model.Biomass, model.Cuts, mutable=True)
    ...
    for i in range(Param.stages, 0, -1): # debug - stages
        ...
        for z in range(iteration+1): 
            #model.Cuts.add(z+1)
            if i == Param.stages:
                model.constTerm[z+1] = fcf_backward[i][0][3]
                ...
                for y, plant in enumerate(biomassPlants):
                    model.coefcBio[plant, z+1] = fcf_backward[i][0][2][y]
            else:
                model.constTerm[z+1] = fcf_backward[i][z][3]
                for y, plant in enumerate(batteries):
                    model.coefcBatt[plant, z+1] = fcf_backward[i][z][1][y]
                for y, plant in enumerate(hydroPlants):
                    model.coefcTerm[plant, z+1] = fcf_backward[i][z][0][y]
                for y, plant in enumerate(biomassPlants):
                    model.coefcBio[plant, z+1] = fcf_backward[i][z][2][y]
        ...
        for z, plant in enumerate(batteries):
            model.maxlvl[plant] = db_storage[z][i-1][1]
            model.maxlvlB[plant] = db_storage[z][i-1][0]*blocksdata[0][y]*b_storageData[z][y]
